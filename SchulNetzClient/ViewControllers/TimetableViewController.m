...
-(void)fetchAndReloadSchedule{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if([Util checkConnection]){
            if(self->running){
                while(self->running && [original compare:self->timetableDate] == NSOrderedSame){
                    NSDate *date = [[NSDate alloc] initWithTimeIntervalSinceNow:0.1];
                    [[NSRunLoop currentRunLoop] runUntilDate:date];
                }
                
                if(self->running || [original compare:self->timetableDate] != NSOrderedSame) return;
            }
            
            self->running = true;
            
            NSObject* doc = [[Variables get].account loadPage:@"22202"];
            
            if(doc && [doc class] == [HTMLDocument class]) [Parser parseSchedulePage:(HTMLDocument*)doc forUser:[Variables get].user];
            
            doc = [[Variables get].account loadScheduleFrom:original to:original];
...
-(void)reload{
    _timetableView.lessons = [self calculateLayout:lessons];
    
    if(_timetableView.lessons.count <= 0){
        _timetableView.hidden = true;
